...
public class filtering {
    ...
    public static void main(String[] args) {
        try (Playwright obj_playwrite = Playwright.create()) {
            ...
            Locator allRows = obj_ControlsPage.locator("#studentTableBody tr");
            System.out.println("Populated student rows count: " + allRows.count());

            allRows.filter(new Locator.FilterOptions().setHasText("Rushikesh")).locator("input[type='number']").fill("60");
            allRows.filter(new Locator.FilterOptions().setHasText("Divyanshu")).locator("input[type='number']").fill("35");
            allRows.filter(new Locator.FilterOptions().setHasText("Vansaj")).locator("input[type='number']").fill("95");
            allRows.filter(new Locator.FilterOptions().setHasText("Shankar")).locator("input[type='number']").fill("75");
